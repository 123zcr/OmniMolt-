...
// Audio formats that need conversion for whisper.cpp (doesn't support AMR/SILK directly)
const UNSUPPORTED_AUDIO_FORMATS = new Set([".amr", ".silk", ".slk"]);

/**
 * Check if audio file needs format conversion for whisper.cpp
 */
function needsAudioConversion(filePath: string): boolean {
  const ext = path.extname(filePath).toLowerCase();
  return UNSUPPORTED_AUDIO_FORMATS.has(ext);
}

/**
 * Check if file is likely SILK format (QQ/WeChat voice messages)
 * SILK files may have .amr, .silk, or .slk extension
 */
function isSilkFormat(filePath: string): boolean {
  const ext = path.extname(filePath).toLowerCase();
  return ext === ".silk" || ext === ".slk" || ext === ".amr";
}

/**
 * Find silk-v3-decoder binary for decoding QQ/WeChat SILK audio
 */
async function findSilkDecoder(): Promise<string | null> {
  // Check common names in PATH
  const names = ["silk_v3_decoder", "silk-v3-decoder", "decoder"];
  ...
  if (process.platform === "win32") {
    ...
    const commonPaths = [
      path.join(homeDir, "Desktop", "silk-v3-decoder", "windows", "silk_v3_decoder.exe"),
      path.join(homeDir, "Desktop", "silk-v3-decoder", "silk_v3_decoder.exe"),
      path.join(homeDir, "Desktop", "moltbot-main", "Release", "silk_v3_decoder.exe"),
      "C:\\silk-v3-decoder\\silk_v3_decoder.exe",
    ];
    
    for (const decoderPath of commonPaths) {
      if (await fileExists(decoderPath)) {
        return decoderPath;
      }
    }
  }
...
/**
 * Find ffmpeg binary - check common locations on Windows
 */
async function findFfmpeg(): Promise<string | null> {
  ...
  if (process.platform === "win32") {
    const homeDir = os.homedir();
    const commonPaths = [
      // Desktop ffmpeg builds
      path.join(homeDir, "Desktop", "ffmpeg-8.0.1-essentials_build", "bin", "ffmpeg.exe"),
      // Other common locations
      path.join(homeDir, "Desktop", "ffmpeg", "bin", "ffmpeg.exe"),
...
/**
 * Decode SILK audio file to PCM using silk-v3-decoder
 */
async function decodeSilkToPcm(params: {
  inputPath: string;
  outputDir: string;
  timeoutMs: number;
}): Promise<string | null> {
  const { inputPath, outputDir, timeoutMs } = params;
  
  const silkDecoder = await findSilkDecoder();
  if (!silkDecoder) {
    logVerbose("SILK decoder not found, cannot decode QQ voice message");
    return null;
  }
  
  const baseName = path.parse(inputPath).name;
  const pcmPath = path.join(outputDir, `${baseName}.pcm`);
  
  try {
    // silk_v3_decoder input.silk output.pcm
    await runExec(silkDecoder, [inputPath, pcmPath], { timeoutMs });
    
    if (await fileExists(pcmPath)) {
      logVerbose(`SILK decoded: ${inputPath} -> ${pcmPath}`);
      return pcmPath;
    }
  } catch (err) {
    logVerbose(`SILK decoding failed: ${String(err)}`);
  }
 ...
 */
async function convertPcmToWav(params: {
  inputPath: string;
  outputPath: string;
  timeoutMs: number;
  ffmpegBinary: string;
}): Promise<boolean> {
  const { inputPath, outputPath, timeoutMs, ffmpegBinary } = params;
  
  try {
    // PCM from SILK decoder is typically 24kHz mono 16-bit
    await runExec(ffmpegBinary, [
      "-f", "s16le",       // signed 16-bit little-endian
      "-ar", "24000",      // 24kHz (SILK default)
      "-ac", "1",          // mono
      "-i", inputPath,
    ...
    logVerbose(`PCM to WAV conversion failed: ${String(err)}`);
    return false;
  }
}

/**
 * Convert audio file to WAV format using ffmpeg (with SILK support via silk-v3-decoder)
 * Returns the path to the converted file, or the original path if conversion not needed/failed
 */
async function convertAudioToWav(params: {
  inputPath: string;
  outputDir: string;
  timeoutMs: number;
}): Promise<{ path: string; converted: boolean; cleanup?: () => Promise<void> }> {
  const { inputPath, outputDir, timeoutMs } = params;
  
  if (!needsAudioConversion(inputPath)) {
    return { path: inputPath, converted: false };
  }
  
  // Check if ffmpeg is available
  const ffmpegBinary = await findFfmpeg();
  if (!ffmpegBinary) {
    logVerbose("Audio conversion needed but ffmpeg not found, trying original file");
    return { path: inputPath, converted: false };
  }
  ...
  // For SILK format (QQ/WeChat voice), need to decode first
  if (isSilkFormat(inputPath)) {
    const pcmPath = await decodeSilkToPcm({ inputPath, outputDir, timeoutMs });
    if (pcmPath) {
      const success = await convertPcmToWav({
        inputPath: pcmPath,
        outputPath,
        timeoutMs,
        ffmpegBinary,
      ...
      if (success) {
        logVerbose(`SILK audio converted: ${inputPath} -> ${outputPath}`);
        return { path: outputPath, converted: true };
      }
    }
    
    // If SILK decoding failed, try direct ffmpeg (might work for real AMR files)
    logVerbose("SILK decoding failed, trying direct ffmpeg conversion");
  }
  
  try {
    // Try direct conversion with ffmpeg (for real AMR or other formats)
    await runExec(ffmpegBinary, [
      "-i", inputPath,
    ...
    if (await fileExists(outputPath)) {
      logVerbose(`Audio converted: ${inputPath} -> ${outputPath}`);
      return { path: outputPath, converted: true };
    }
  } catch (err) {
    logVerbose(`Audio conversion failed: ${String(err)}, using original file`);
  }
...
async function resolveLocalWhisperCppEntry(): Promise<MediaUnderstandingModelConfig | null> {
  // Check for whisper-cli in PATH or in local Release folder
  const releaseDir = path.join(path.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, "$1")), "..", "..", "Release");
  const releaseBin = path.join(releaseDir, process.platform === "win32" ? "whisper-cli.exe" : "whisper-cli");
  const whisperBinary = (await fileExists(releaseBin)) ? releaseBin : (await hasBinary("whisper-cli")) ? "whisper-cli" : null;
  if (!whisperBinary) return null;

  // Check for model: env var > Release folder > default macOS path
  const envModel = process.env.WHISPER_CPP_MODEL?.trim();
  const releaseModel = path.join(releaseDir, "ggml-base.bin");
  const defaultModel = "/opt/homebrew/share/whisper-cpp/for-tests-ggml-tiny.bin";
  const modelPath = (envModel && (await fileExists(envModel)))
    ? envModel
    : (await fileExists(releaseModel))
      ? releaseModel
      : defaultModel;
  if (!(await fileExists(modelPath))) return null;
  // Use Chinese language by default for better accuracy with Chinese audio
  const language = process.env.WHISPER_LANGUAGE?.trim() || "zh";
  return {
    type: "cli",
    command: whisperBinary,
    args: ["-m", modelPath, "-l", language, "-otxt", "-of", "{{OutputBase}}", "-np", "-nt", "{{MediaPath}}"],
  };
}
...
async function runCliEntry(params: {
  ...
  // For audio capability, convert unsupported formats (AMR/SILK) to WAV
  if (capability === "audio") {
    const conversionResult = await convertAudioToWav({
      inputPath: mediaPath,
      outputDir,
      timeoutMs,
    });
    mediaPath = conversionResult.path;
    // Note: converted file is in outputDir, will be cleaned up with outputDir
  }
