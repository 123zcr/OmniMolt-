...
/**
 * Clean text for TTS output - remove markdown formatting and emoji/sticker tags
 * that should not be spoken aloud
 */
function cleanTextForTts(text: string): string {
  let cleaned = text;
  
  // Remove emoji/sticker tags like [表情:xxx] [表情：xxx] [emoji:xxx]
  cleaned = cleaned.replace(/\[表情[:：][^\]]*\]/gi, "");
  ...
  cleaned = cleaned.replace(/\|/g, ""); // table pipes
...
export async function maybeApplyTtsToPayload(params: {
  ...
  const maxLength = getTtsMaxLength(prefsPath);
  let textForAudio = cleanTextForTts(ttsText.trim());
  ...
  if (result.success && result.audioPath) {
    ...
    const voiceOnly = config.voiceOnly === true;
    const finalPayload = {
      ...nextPayload,
      // When voiceOnly is true, omit text from the payload (send voice only)
      text: voiceOnly ? undefined : nextPayload.text,
