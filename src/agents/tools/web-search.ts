...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  if (provider === "tavily") {
    return {
      ...
      message: `web_search (tavily) needs an API key. Set TAVILY_API_KEY in the Gateway environment, or configure tools.web.search.apiKey.`,
      docs: "https://docs.molt.bot/tools/web",
    };
  }
  return {
...
async function runWebSearch(params: {
  ...
  if (params.provider === "tavily") {
    const tavilyData = await runTavilySearch({
      query: params.query,
      apiKey: params.apiKey,
      count: params.count,
      timeoutSeconds: params.timeoutSeconds,
    ...
    const results = (tavilyData.results ?? []).map((entry) => ({
      title: entry.title ?? "",
      url: entry.url ?? "",
...
export function createWebSearchTool(options?: {
  ...
  const description =
    provider === "perplexity"
      ...
      : provider === "tavily"
        ? "Search the web using Tavily API. Returns AI-synthesized answers with search results including titles, URLs, and content snippets."
